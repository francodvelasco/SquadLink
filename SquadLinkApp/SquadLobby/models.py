...
class SquadLinkLobbyManager(models.Manager):
    def create(self, *args, **kwargs):
        ...
        if user and form:
            ...
            kwargs['platforms'] = ', '.join(
                map(lambda short: platform_dict[short], form.cleaned_data.get('platforms')))
...
class SquadLinkLobby(models.Model):
    ...
    squad_members = models.ManyToManyField(
        SquadLinkUserModel, related_name='members', blank=True)
    description = models.CharField(max_length=280)
    photo = models.ImageField(
        upload_to=lobby_image_location, null=True, blank=True)
    ...
    def update_from_form(self, form: LobbyCreateForm):
        if form and isinstance(form, LobbyCreateForm):
            ...
            platform_dict = dict(LobbyCreateForm.PLATFORMS)
            form_content['platforms'] = ', '.join(
                map(lambda short: platform_dict[short], form_content.get('platforms')))
